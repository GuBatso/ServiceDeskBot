...
| 4 | Backend | 🔄 Núcleo funcional concluído — solução multi-projeto, modelo de domínio (10 entidades + 2 enums), `AppDbContext` registrado via DI, connection string via User Secrets, primeira Migration gerada e **aplicada com sucesso** (tabelas criadas no PostgreSQL). **Pendente:** classes de Fluent API (`IEntityTypeConfiguration<T>`) para índices únicos, enums como texto e tamanhos de coluna — atualmente as tabelas usam apenas convenções padrão do EF Core |
...
## 14. Referência de comandos de terminal

Todos os comandos `dotnet`/`psql` utilizados na construção do projeto (criação da solução, referências entre projetos, pacotes NuGet, User Secrets, criação do banco) estão documentados separadamente em `comandos-cli-projeto.md`, para consulta rápida sem precisar navegar por este documento inteiro.

## 15. Estado atual e próximo passo
...
**Próximo passo:** implementar as classes de configuração Fluent API (`IEntityTypeConfiguration<T>`) por entidade — cobrindo índices únicos (`Usuario.Telefone`, `Chamado.NumeroChamado`, `Tecnico.Email`), conversão dos enums `StatusChamado`/`PrioridadeChamado` para texto em vez de número, e `MaxLength` nos campos de texto — seguido de uma nova Migration para aplicar esses ajustes. Depois disso, seguir para a Fase 5 (Sistema de Chamados) e acrescentar comentários explicativos no código já escrito.
