...
Sistema de service desk de TI operado via WhatsApp, atendendo colaboradores de três ambientes: **Clínica**, **Hospital** e **Escritório**. O colaborador abre o chamado conversando com o chatbot; o técnico gerencia, diagnostica e encerra o chamado via API/painel (Planejo construir um site para visualizar o historico de chamados).

O projeto tem duplo objetivo: (1) ser uma ferramenta funcional para o dia a dia de suporte técnico do autor e (2) servir como projeto de aprendizado estruturado de arquitetura de software, aplicando conhecimento prévio em C#/.NET, JWT, EF Core e PostgreSQL.
...
| 4 | Backend | 🔜 Próxima — tarefa em andamento: POCOs de `Local`, `Setor`, `Categoria`, `Subcategoria` |
...
As Fases 1 a 3 estão concluídas e documentadas acima. O passo imediato em andamento é uma tarefa prática: escrever as classes POCO em C# de `Local`, `Setor`, `Categoria` e `Subcategoria`, com base no modelo de dados da seção 11, antes de avançar para `Equipamento` e `Chamado` (entidades com mais relacionamentos e regras de negócio).
